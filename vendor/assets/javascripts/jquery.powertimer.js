...
(function($) {
    ...
    $.fn.extend({
        powerTimer: function(options, timerName) {
    ...
    function registerTimeout(elem, options, duration) {
        if (duration > 0) { // call later
            setTimerValue(elem, options.name, 'timerid', window.setTimeout(function() {
    ...
    function callWithElement(elem, options) {
        ...
        // call the function and stop any extra timers that got placed on it (so we don't double them)
        var ret = options.func.call(elem, options.params);
        stopTimerByName(elem, options.name);

        // restart the timer if we're supposed to
